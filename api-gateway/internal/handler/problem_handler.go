...
func RegisterRoutes(
	...
	if adminH != nil {
		for _, prefix := range []string{"/api/v1/admin", "/api/admin", "/admin"} {
			...
			g.GET("/modules", adminH.ListAdminModules)
			g.POST("/modules", adminH.CreateModule)
			g.PUT("/modules/:id", adminH.UpdateModule)
			g.DELETE("/modules/:id", adminH.DeleteModule)
			g.PUT("/modules/:id/reorder", adminH.ReorderModuleProblems)
